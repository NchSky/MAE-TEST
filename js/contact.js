...
function check_values() {
	...
	var errors=0;
	 ...
	 if($j('#contactform #first-name').val()=='') {
	 	...
	 	if(!hasClass)
	 	    $j('#contactform #first-name').parent().append('<label for="contactname" generated="true" class="error">Tienes que ingresar un nombre!</label>');
		...
		if($j('#contactform #last-name').val()!=undefined)
	 	...
	 	if(!hasClass)
	 	    $j('#contactform #last-name').parent().append('<label for="contactname" generated="true" class="error">Tienes que ingresar un apellido!</label>');
		...
		var hasClass=$j('#contactform #email').parent().find(".error").hasClass("error");
	 	if(!hasClass)
	 	    $j('#contactform #email').parent().append('<label for="contactname" generated="true" class="error">Ingresa un email valido!</label>');	
		...
		var hasClass=$j('#contactform #subject').parent().find(".error").hasClass("error");
	 	if(!hasClass)
	 	    $j('#contactform #subject').parent().append('<label for="contactname" generated="true" class="error">Tienes que ingresar un asunto!</label>');	
		...
		var hasClass=$j('#contactform #message').parent().find(".error").hasClass("error");
	 	if(!hasClass)
	 	    $j('#contactform #message').parent().append('<label for="contactname" generated="true" class="error">Tienes que escribir un mensaje!</label>');	
